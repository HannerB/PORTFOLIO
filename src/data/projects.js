...
export const projects = [
    ...
    },
    {
        ...
        features: [
            {
                ...
                description: "Courses are organized into modules and lessons. Each lesson streams via the Vimeo Player API. When a video ends, a countdown timer appears and automatically advances to the next lesson — skippable at any point. Lesson completion is recorded in Supabase and guarded against duplicate writes.",
            },
            {
                title: "Stripe checkout with HMAC webhook verification",
                description: "A one-time payment (107 EUR) grants full platform access. Stripe collects customer email, phone and billing address at checkout, plus explicit Terms/AUP/EULA acceptance. A webhook endpoint validates the Stripe-Signature header before processing, and the Stripe session ID acts as an idempotency key to handle retries safely.",
            },
            {
                title: "Digital library with in-browser PDF reader",
                description: "Students browse a catalog of books, view metadata (author, categories, duration), and read them directly in the browser using react-pdf. Downloads are supported. Public visibility mode lets unauthenticated visitors preview content before purchasing.",
            },
            {
                title: "Role-based access control and NestJS course guards",
                description: "Supabase RLS policies enforce row-level data isolation at the database layer. On the backend, NestJS guards validate JWT tokens and verify course-access records before serving any lesson or protected resource — no client-side access control is relied upon.",
            },
            {
                title: "Admin dashboard — full CRUD",
                description: "Admins manage the entire platform from a dedicated dashboard: create, edit and delete users, courses, modules, lessons, books and categories; broadcast notifications; review payment records; toggle promotional pricing; and ban users.",
            },
            {
        ...
        stack: {
            ...
            styling: ["Tailwind CSS 3", "Radix UI", "Framer Motion 12", "Lucide React"],
            forms: ["React Hook Form", "Zod"],
            media: ["Vimeo Player API", "react-pdf / pdfjs-dist", "Recharts"],
        ...
        demoNote: {
            ...
            steps: [
                { label: "Landing page", detail: "Browse the marketing site — pricing, course catalog and public book preview.", link: "https://1tomillion.com" },
                { label: "Platform demo", detail: "Log in with demo@1tomillion.com / 123456 — explore the dashboard, course player, Vimeo autoplay, progress tracking and digital library.", link: "https://1tomillion.com/login" },
            ],
        },
    },
    {
        slug: "proveify-back",
        category: "platform",
        year: "Apr 2025 – Feb 27, 2026",
        title: "Proveify",
        tagline: "Backend support on a B2B service-provider marketplace — quotes, payments and domain modules from scratch",
        image: "https://picsum.photos/seed/proveify/1200/600",
        screenshots: [],
        github: "https://github.com/proveify/proveify-back",
        githubPrivate: false,
        link: "https://provefly.netlify.app/",
        tags: ["NestJS", "Prisma", "TypeScript", "PostgreSQL", "Wompi", "JWT", "Swagger", "Jest"],
        overview:
            "Proveify is a B2B marketplace where companies post public service requests and verified providers submit quotes. I joined the team as backend support and owned several core domain modules end-to-end: categories, user profile, favorites, public requests, the full quoting system (both client and provider sides), an optional-auth guard that unified duplicate endpoints, and the Wompi payment gateway integration.\n\nThe codebase follows a strict NestJS modular architecture: each domain has its own controller, service, abstract repository interface, Prisma implementation, DTOs, entities and factories. The Prisma schema is split one file per domain, keeping the data layer isolated and navigable.",
        problem:
            "The marketplace needed several domain modules built from zero while conforming to a consistent architecture already in use across the project: repository pattern with abstract interfaces, Swagger decorators kept out of controllers, serialization via entity classes and unit tests for every controller and service. Beyond implementing features, one key challenge was the quoting flow: providers and clients needed to exchange quotes in a way that worked for both authenticated users and anonymous visitors — requiring a smart authentication strategy rather than two separate endpoint trees.",
        role: "Backend support developer. Joined an existing NestJS project and implemented the following modules independently: categories and subcategories (models, seeds, exceptions, Swagger), user profile retrieval with entity serialization, the full favorites system (later refactored into the items module), PublicRequest CRUD with repository pattern, the complete quotes system (Prisma schema, DTOs, CRUD, unit tests, Swagger), OptionalJwtAuthGuard to unify public/authenticated endpoints, provider-side quotes with factory pattern, unification of the quotes and provider-quotes modules, and the Wompi payment gateway (Prisma schema, SHA256 integrity signatures, webhook verification, full Swagger docs).",
        features: [
            {
                title: "Wompi payment gateway",
                description: "Full payment module following the project architecture: Prisma schema with PaymentStatus enum and relations to Quotes and Users, Joi-validated config for 5 env vars, repository, factory, service and controller. WompiService generates SHA256 integrity signatures and verifies webhook event authenticity before updating payment state. Endpoints: POST /payments, POST /payments/webhook, GET /payments/:reference — all documented with Swagger decorators.",
            },
            {
                title: "Complete quotes system",
                description: "Designed the Prisma schema for Quotes and QuoteItems, built the full CRUD (QuotePrismaRepository, QuoteService, QuoteController), wrote 250 lines of Swagger documentation via custom decorators, and added unit tests for both controller and service. Supports anonymous quote creation with provider-existence validation before insert.",
            },
            {
                title: "OptionalJwtAuthGuard — unified endpoints",
                description: "Implemented OptionalJwtAuthGuard and OptionalJwtStrategy so a single endpoint serves both unauthenticated and authenticated callers. Removed the duplicate /authenticated variants across items and providers, halving the API surface while preserving personalized responses for logged-in users.",
            },
            {
                title: "PublicRequest module",
                description: "Built the full PublicRequest CRUD from scratch: Prisma model, DTOs, entity, service, controller and 231 lines of Swagger documentation. Refactored the underlying data-access class to follow the project's repository-pattern convention (TransactionPrismaService → PublicRequestPrismaRepository).",
            },
            {
                title: "Repository pattern rollout",
                description: "Refactored 7 existing services (Category, File, Item, Plan, Provider, Subcategory, User) to depend on abstract repository interfaces backed by Prisma implementations — decoupling business logic from the ORM and making each service independently testable.",
            },
            {
                title: "Favorites system",
                description: "Implemented the Favorites module with full CRUD and unit tests, then refactored it into the Item module so favorite state is returned inline on item queries for authenticated users — eliminating a separate round trip.",
            },
            {
                title: "Unit test coverage",
                description: "Wrote Jest unit tests for every module I owned: UserController, UserService, CategoryController, ItemService (with favorites), FavoriteController, FavoriteService, PublicRequestController, PublicRequestService, QuotePrismaRepository, QuoteService, QuoteController, ProviderQuoteController and ProviderQuoteService.",
            },
        ],
        stack: {
            backend: ["NestJS 11", "TypeScript", "Passport.js", "JWT", "nestjs-cls"],
            database: ["PostgreSQL", "Prisma ORM 6", "Prisma multi-file schema", "Prisma Seeding"],
            payments: ["Wompi", "SHA256 integrity signatures", "Webhook verification"],
            storage: ["Google Cloud Storage", "nestjs-form-data"],
            tooling: ["Swagger / OpenAPI", "Jest", "Joi", "Docker", "docker-compose"],
        },
        highlights: [
            "Wompi webhook handler recomputes the SHA256 integrity signature server-side before trusting any event — a forged request with a mismatched signature is rejected before touching the database.",
            "OptionalJwtStrategy returns null (not an error) on a missing or invalid token, letting controllers decide whether user context is required — same guard, two behaviors, no duplicated endpoints.",
            "Abstract repository interfaces mean QuoteService depends on IQuoteRepository, not PrismaClient directly. Swapping the data source or mocking in tests requires zero changes to business logic.",
            "Favorites are returned as an inline boolean on every item query for authenticated users — resolved in one SQL join rather than a second HTTP call from the client.",
        ],
    },
